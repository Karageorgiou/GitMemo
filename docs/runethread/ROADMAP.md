...
- the existing private evidence-storage boundary also carries a minimized append-only rollback-independent safety journal rather than a second queue/state machine. Binding/enrollment, client-visible acceptance, cancellation wins, accepted canonical anchors, publication intents/outcomes, required immutable receipt references, and independently verified terminal-success receipt references/result identities are create-if-absent/digest-bound and contain no plaintext memory bodies or credentials;
- Durable Object PITR/recreation/detected state rollback is an exclusive maintenance/recovery barrier, not an ordinary retry. A restored/recreated DO must reconcile its journal checkpoint/epoch, immutable evidence, current GitHub binding/privacy, accepted operations/cancellations, protected publication anchors, terminal-success evidence, and possibly-issued publication capabilities before normal admission/publication resumes;
...
- unsuccessful/stale finalizer results remain generation-bound/fail-closed but do not require equivalent independent semantic replay solely to defend against denial of service by a compromised finalizer;
- canonical repository/trust/compatibility/ref-binding failure separated from request-local mutation failure;
- candidate evidence bound to repo/canonical-ref/H0/C/tree/parent/Core-idempotency/request fingerprint/hosted attempt+generation/runtime/container/delivery/contract/candidate-envelope identities and digests;
...
- immutable generation-bound candidate audit receipt binds exact request digest/fingerprint, H0, C/tree, independently derived semantic + commit-envelope conformance, and pinned runtime/delivery/contract/envelope identities, with deterministic audit failure durably suspending/reconciling lane before release;
...
- explicit release/protocol versioning and non-atomic Worker/DO/finalizer/auditor-verifier/evidence/publication/reconciliation/privacy/managed-bootstrap/canonical-ref/safety-journal/recovery/audit-conformance/terminal-success-verification/candidate-envelope deployment handling using maintenance/drain or versioned blue/green;
- operation outcomes `NO_OP`, `ALREADY_COMMITTED`, `NEEDS_REPREPARE`, finalization/audit/cancel/reconciliation plus lane OPEN/SUSPENDED/MAINTENANCE, with `NO_OP`/`ALREADY_COMMITTED` not authoritative until ADR-021 verification succeeds;
- control-plane barriers for contract/schema/trust/repository-format/bootstrap/delivery/migration/evidence/publication/reconciliation/privacy/canonical-ref binding/recovery/audit-conformance/terminal-success-verification/candidate-envelope changes;
...
- measure cold/warm source, bytes, idempotency/stale preflight, finalization, deterministic candidate-object construction, package, request-conformance + raw-commit-envelope audit, terminal-success verification, publication, publisher/API-path startup/fencing, journal recovery, alarm/retry/interleaving, provider startup, total latency/cost.
...
The current full attack review, explicitly started against synchronized head `9151c9d2e1a383e79449af2963fc2c547bb49429`, confirmed ADR-021 closes the terminal-success bypass but found that ADR-020 still left ambient/nondeterministic Git commit metadata outside the independently derived semantic tree/message proof. Current Core sets Git identity by config during `git commit`, while Git author/committer environment variables can override those values; a compromised finalizer could therefore keep the exact authorized semantic tree/message while persisting attacker-chosen identity or private bytes in exact canonical candidate `C`. ADR-022 makes the complete candidate commit envelope deterministic/Core-owned, independently derives exact expected C, and prevents unrelated unreachable Git objects from becoming published storage merely because the finalizer packaged them. **This review therefore also fails the zero-edit freeze gate.** No further review run is started in the same prompt after these edits; implementation remains blocked until a later explicitly requested full review of the new exact synchronized head passes with zero edits.
...
- explicit PITR/recreation recovery reconstructs client-visible accepted operations, cancellation wins, accepted canonical anchors, independently verified terminal-success outcomes, and possible/proven publication history before normal writes resume;
...
- deployment/version barriers prevent mixed incompatible finalizer/auditor-verifier/evidence/publication/reconciliation/privacy/managed-bootstrap/safety-journal/recovery/audit-conformance/terminal-success-verification/candidate-envelope semantics;
...
> Finish the pre-implementation architecture freeze, then implement the ADR-015 contract + managed-support migration followed by the Cloudflare-hosted, audited, per-repository mutation-delivery path defined by ADR-012 through ADR-022. Normal hosted writes begin only on an explicitly compatible migrated private repository. Reconciliation preserves accepted Git/idempotency history and every proven/possibly published candidate. A minimized append-only safety journal prevents destructive Durable Object rollback/recreation from forgetting accepted promises, cancellations, accepted anchors, publication intents, or independently verified terminal-success outcomes without becoming a second live state machine. Candidate success crosses ADR-020's Core-owned exact request+H0 semantic conformance audit and ADR-022's deterministic full commit-envelope proof, so the exact candidate object `C` is independently reproducible and cannot carry ambient author/committer metadata or unrelated packaged objects. Successful no-candidate `NO_OP`/`ALREADY_COMMITTED` claims cross ADR-021's fresh Core/repository verification so the finalizer cannot bypass the independent boundary by falsely declaring success. Canonical publication uses only the verified exact-C object closure plus a true expected-old Git update with publication-capability fencing; the current GraphQL expected-old path remains an implementation prototype, with the minimal Git publisher retained as safe fallback until exact-object identity is proven. The review against `9151c9d2…` found the ADR-022 candidate-envelope gap, so implementation remains blocked until a later zero-edit full review of the new exact planning head passes.
