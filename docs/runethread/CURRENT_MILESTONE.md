...
3. keep Core idempotency identity separate from hosted operation-attempt identity; hosted attempt identity binds immutable repository identity plus the exact sealed-request digest so byte-different envelopes sharing one Core key cannot alias one Workflow, while Core remains authoritative for semantic equivalence/conflict;
4. implement one stateful coordinator per immutable GitHub repository identity using a Cloudflare Durable Object, with explicit lane states including `OPEN`, `SUSPENDED`, `MAINTENANCE`, and reconciliation;
5. serialize the entire heavy finalization/audit/publication workflow per repository in v1, but preserve ADR-003 ordering: canonical Core/Git committed-idempotency lookup happens before stale classification, so stale work is guaranteed to stop before candidate construction/Index write/package/audit, not necessarily before cold Container/source acquisition;
6. use a deterministic attempt-bound Workflow instance identity plus crash-safe get-or-create reconciliation so a crash between Durable Object state and Workflow creation cannot start duplicate heavy workflows or strand the lane;
7. use one Cloudflare Workflow per admitted operation for durable, idempotent checkpoints/retries while keeping the Durable Object as the sole repository-lane and publication authority;
8. isolate the GitHub App private key in a private internal GitHub gateway/publisher Worker; the public API has no publication binding, the baseline runtime App requests only minimum Contents/Metadata authority, not Administration or Workflows permission, and finalizer/auditor receive only narrow short-lived read tokens when required;
9. run the existing deterministic Runethread Go/Core + Git finalizer in a Cloudflare Container; a cold finalizer targets at most one GitHub source clone/fetch and a warm clone may be reused only as an untrusted disposable cache after exact refresh/reset to direct remote canonical state;
10. never retry `ApplyMutation` against a surviving local unpromoted candidate branch: every fresh finalization attempt starts from direct remote canonical state, because local `C` already contains operation metadata and must not be mistaken for canonical committed evidence;
11. make the hosted finalization step itself idempotent: after `ApplyMutation` succeeds, persist complete exact candidate evidence first and create an immutable attempt-bound finalization receipt last; conditional receipt creation selects the one authoritative candidate if overlapping retries raced, and a retry uses that receipt or resets to remote canonical state when no receipt exists;
12. let `ApplyMutation` preserve its existing committed-idempotency-before-stale ordering and perform candidate construction, one Index v2 write, hard validation, and local-only fast-forward rather than recreating those semantics in provider code; transport hashes bind artifacts but do not replace Core's semantic request fingerprint;
13. treat `NO_OP` as a Core-validated terminal outcome that skips candidate/audit/publication, not as a provider-side shortcut or invented Git commit;
14. distinguish request-local mutation failure from canonical repository/trust/compatibility failure: an unhealthy accepted canonical base suspends/reconciles the lane instead of producing endless unrelated `FINALIZATION_FAILED` operations;
15. export exact candidate `C` plus its operation/base/runtime/contract bindings as private content-addressed, digest-verified, no-overwrite candidate evidence; receipt/evidence mismatch is an integrity failure, while orphan package data without a receipt is nonauthoritative and may be garbage-collected;
16. independently audit exact `C` in a fresh reduced-privilege Container with hard validation, strict Index v2 freshness, candidate/request/runtime binding, and expected-diff checks, using only the minimum read-only base fetch needed to reconstruct `C`;
17. deterministic audit failure must become durable lane suspension or conservative reconciliation before the active lane can be released;
18. after audit, return exact evidence to the repository Durable Object; only that lane authority may atomically transition the active operation from `AUDITED` to `PUBLISHING` after rechecking cancellation, suspension, maintenance, operation identity, and evidence binding;
19. publish only the exact DO-authorized audited `C` through the private GitHub gateway and an atomic expected-old-revision compare-and-swap; prototype clone-free Git-object publication but fall back to exact candidate push if exact commit identity cannot be reproduced;
20. once `PUBLISHING` wins, cancellation is no longer a correctness mechanism; after a crash/response loss resolve exact Git state (`C`, `H0`, or unexpected/reconciliation) and retry only the same exact authorized CAS when appropriate;
21. after successful publication, confirm `main == C` cheaply rather than running another full synchronous clone/validation cycle;
22. subscribe the GitHub App to signed `push` webhooks only for fast observation; every webhook is a hint that triggers a direct canonical-ref read, and stale/out-of-order webhook payloads never directly mutate accepted repository state;
23. distinguish proven uncommitted stale work (`NEEDS_REPREPARE`) from unexpected movement during the one active hosted workflow (`RECONCILIATION_REQUIRED`);
24. keep GitHub Free and paid private repositories on the same normal hosted mutation architecture, using paid branch/ruleset protection only as optional defense-in-depth;
25. preserve singleton operations, exact committed retry, cancellation-before-publication, audit-failure suspension, explicit recovery, and exclusive control-plane barriers;
...
29. roll the finished mechanism through the required release/downstream gates and a real private memory repository, then measure cold/warm source acquisition, bytes transferred, idempotency/stale preflight, finalization, packaging, audit, publication, and end-to-end latency/cost separately.
