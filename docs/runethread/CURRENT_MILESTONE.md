...
5. serialize the entire heavy finalization/audit/publication workflow per repository in v1, so known distinct queued stale operations become `NEEDS_REPREPARE` before starting a Container without violating Core's committed-idempotency-before-stale rule during ambiguous recovery;
...
10. never retry `ApplyMutation` against a surviving local unpromoted candidate branch: every fresh finalization attempt starts from direct remote canonical `H0`, because local `C` already contains operation metadata and must not be mistaken for canonical committed evidence;
11. make the hosted finalization step itself idempotent: after `ApplyMutation` succeeds, persist complete exact candidate evidence first and create an immutable attempt-bound finalization receipt last; a retry uses that receipt, or if none exists resets to remote `H0` and re-finalizes;
...
14. export exact candidate `C` plus its operation/base/runtime/contract bindings as private content-addressed, digest-verified, no-overwrite candidate evidence; receipt/evidence mismatch is an integrity failure, while orphan package data without a receipt is nonauthoritative and may be garbage-collected;
15. independently audit exact `C` in a fresh reduced-privilege Container with hard validation, strict Index v2 freshness, candidate/request/runtime binding, and expected-diff checks, using only the minimum read-only base fetch needed to reconstruct `C`;
16. deterministic audit failure must become durable lane suspension or conservative reconciliation before the active lane can be released;
17. after audit, return exact evidence to the repository Durable Object; only that lane authority may atomically transition the active operation from `AUDITED` to `PUBLISHING` after rechecking cancellation, suspension, maintenance, operation identity, and evidence binding;
18. publish only the exact DO-authorized audited `C` through the private GitHub gateway and an atomic expected-old-revision compare-and-swap; prototype clone-free Git-object publication but fall back to exact candidate push if exact commit identity cannot be reproduced;
19. once `PUBLISHING` wins, cancellation is no longer a correctness mechanism; after a crash/response loss resolve exact Git state (`C`, `H0`, or unexpected/reconciliation) and retry only the same exact authorized CAS when appropriate;
...
22. distinguish known queued staleness (`NEEDS_REPREPARE`) from unexpected movement during the one active hosted workflow (`RECONCILIATION_REQUIRED`);
...
25. enforce explicit hosted request/repository/artifact/runtime/retry/operation-history limits plus private-data/log/retention controls so quota or provider failures leave canonical Git unchanged;
26. keep project `current-state.md`/orientation prose outside the atomic memory dual-write transaction; future refresh of those project views is a separate materialized-view/projection concern;
27. remove push-on-every-normal-memory full GitHub Actions validation from the hosted data-plane path through the proper managed workflow/control-plane rollout, while retaining Actions where health/recovery/migration/control-plane checks prove a distinct invariant;
28. roll the finished mechanism through the required release/downstream gates and a real private memory repository, then measure cold/warm source acquisition, bytes transferred, finalization, packaging, audit, publication, and end-to-end latency/cost separately.
